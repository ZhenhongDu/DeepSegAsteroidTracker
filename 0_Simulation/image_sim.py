...
def stars(image, number, fwhm_min=2, fwhm_max=6, fluxes_min=200, fluxes_max=10000):
    """
    ...
    # star intensity range
    flux_range = [fluxes_min, fluxes_max]
    y_max, x_max = image.shape

    # star range
    xmean_range = [0.02 * x_max, 0.98 * x_max]
    ymean_range = [0.02 * y_max, 0.98 * y_max]

    # star size
    xstddev_range = [fwhm_min, fwhm_max]
    ystddev_range = [fwhm_min, fwhm_max]
...
def asteroid(image, x_0, y_0, theta_0, v_0, t, fluxes, fwhm, random_change=True):
    """
    Calculate the position of an asteroid after time t, given its initial position,
    angle of motion, and velocity.

    Parameters:
    x_0 (float): Initial x-coordinate of the asteroid.
    y_0 (float): Initial y-coordinate of the asteroid.
    theta_0 (float): Angle of motion in degrees (0-360°).
    v0 (float): Velocity of the asteroid.
    t (int): Time after which the position is calculated.

    Returns:
    (float, float): The x and y coordinates of the asteroid at time t.
    """
    wid_y, wid_x = np.shape(image)
    # Convert angle from degrees to radians
    theta_rad = np.radians(theta_0)

    # Calculate new position
    pos_x = x_0 + v_0 * t * np.cos(theta_rad)
    pos_y = y_0 + v_0 * t * np.sin(theta_rad)
    pos_y = wid_y - pos_y



    if random_change:
        flux_range = [0.8*fluxes, 1.2*fluxes]
        xmean_range = [0.95 * pos_x, 1.05 * pos_x]
        ymean_range = [0.95 * pos_y, 1.05 * pos_y]
    else:
        flux_range = [fluxes, fluxes]
        xmean_range = [pos_x, pos_x]
        ymean_range = [pos_y, pos_y]
    ...
    sources = make_random_gaussians_table(1, params, seed=12345)
